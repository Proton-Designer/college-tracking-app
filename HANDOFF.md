...
inventory of what is deliberately *not* in git.**
...
pasteable CLI command or a named dashboard control with the exact value. The compressed version:

```bash
supabase login
supabase link --project-ref <PROJECT_REF>
supabase db push --dry-run     # review
supabase db push               # apply the same 33 migrations
...
Then: copy `.env.example` → `.env` and fill it; set Edge Function secrets
(`supabase secrets set ANTHROPIC_API_KEY=…`); create the storage bucket; enable cron.
