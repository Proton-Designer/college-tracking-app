...
| 🟡 S6 | **No automated guard against demo-account contamination** | Test hygiene (S1/S2's pattern, generalized) was fixed file-by-file this session (`focusSessions.itest.ts`, then `killLoop`/`frictionLogging`/`proofOfWork`/`syllabusUpload.itest.ts`) after each one was independently found polluting `demo@collegeos.app` — including a real, live-caught case: 6 `friction_logs` dated 2099 that would have silently poisoned any windowed trend query touching them, with no error anywhere. Fixing tests one at a time stops each *specific* leak, not the *next* one. Needs `scripts/check-demo-clean.mjs` — asserts demo carries no synthetic artifacts (no rows dated beyond today anywhere on the account, no `%test%`/`pow-test-%`-shaped titles, no kill_habits/syllabus_uploads beyond the seeded set) — wired into `npm run verify` alongside `check:imports`/`check:core-mirror`, same rationale: a shared fixture decays silently without an automated check. |
| 🟡 S7 | **No automated guard against unreachable public exports** | Real, working, tested code was unreachable from `@collegeos/api` twice this session — `getNightReviewDraft`/`getPredictionForDate` (blocked Nova for hours before it was caught), then `agentReports`/`summaries`/`insights` from L7 (caught only by inspection, no consumer hit it yet). Needs `scripts/check-barrel-exports.mjs`: walk `packages/api/src/**` and `packages/core/src/**`, find modules exporting public symbols, fail if any aren't re-exported from that package's `index.ts` (allow an explicit opt-out comment for genuinely internal modules). Wire into `verify` alongside `check:imports`/`check:core-mirror`/`check:demo-clean`. Bundle with S6 — same batch. |
