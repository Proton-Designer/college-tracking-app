...
| ~~🔴 L4~~ | ~~No `ANTHROPIC_API_KEY` yet~~ | **Resolved 2026-08-25.** Key set via `supabase secrets set`; §7 checklist executed: 86 offline tests keyless, live smoke OK on all three models with zero fixture drift, `nightly-analysis` ran end-to-end on the deployed function (`usedModel: true`, Sonnet), `llm_usage_log` audited (sha256 content_hash, correct model/call_type, **no prompt text anywhere in the row**), cost math verified against a live bill to the sixth decimal, prompt caching observed working (2363-token cache write then read), and a $0.01 ceiling **blocked a real call** with no provider hit and no log row while still producing the deterministic report. §7 fully closed 2026-08-25: real syllabi extracted with correct titles/weights/dates after the wire-schema fix, announcements parsed and applied, and the ledger audited — 6 rows, correct models per call type, 64-char hashes, no prompt text (content-term regex over serialized rows), $0.098 of the $5.00 ceiling. |
