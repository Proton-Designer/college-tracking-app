...
describe('computeAssignmentRisk', () => {
  ...
  it('returns 100 when every factor is at its ceiling', () => {
    const result = computeAssignmentRisk({
      ...base,
      dueDate: base.today, // overdue/due-today -> proximity = 1
      weightPct: 30, // saturates weight factor
      difficultyRating: 5,
      confidenceRating: 1,
      completedUnits: 0,
      plannedUnits: 0, // "nothing planned" -> unfinished = 1
      committedHours: 10,
      availableHours: 10, // congestion = 1
      userMeanStartDelayDays: 2,
      userStartDelaySampleSize: 5,
      targetPct: 95,
      projectedPct: 90, // headroom = 1
    });
  ...
  it('is monotonic in unfinished work: less progress never scores lower', () => {
    const done = computeAssignmentRisk({
      ...base,
      weightPct: 20,
      dueDate: '2026-08-25',
      completedUnits: 4,
    ...
    const started = computeAssignmentRisk({
      ...base,
      weightPct: 20,
      dueDate: '2026-08-25',
      completedUnits: 1,
  ...
  it('keeps a 30%-weight exam 20 days out out of the critical band even under worst-case other factors', () => {
    const result = computeAssignmentRisk({
      today: '2026-08-18',
      dueDate: '2026-09-07', // 20 days out
      weightPct: 30,
      difficultyRating: 5,
      confidenceRating: 1,
      completedUnits: 0,
  ...
  it('excludes a missing difficulty rating and renormalizes remaining weights, rather than defaulting to 0 or 0.5', () => {
    ...
    // renormalized weights of the remaining factors must still sum to 1
    const remainingWeightSum = withoutRating.trace
      .filter((t) => t.key !== 'difficulty')
      .reduce((s, t) => s + t.weight, 0);
    expect(remainingWeightSum).toBeCloseTo(1, 6);
    // missing data should not be a free discount: the score should not be lower than the
    // known-difficulty case just because difficulty was omitted from the sum.
  ...
  it('falls back to the global mean start delay when the user has fewer than 5 observations', () => {
    ...
    // fewSamples should use the (worse) global mean of 2, not the user's 0
