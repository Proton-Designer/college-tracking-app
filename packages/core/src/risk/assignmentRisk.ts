...
/** Confidence from the weight-mass of factors excluded for missing data. */
...
export interface AssignmentRiskInput {
  ...
  /** Target grade percentage for the course, if the user has set one. */
  targetPct?: number;
  /** Current projected course grade percentage, if computed. */
  projectedPct?: number;
}
...
function normalizeProximity(today: LocalDate, dueDate: LocalDate): number {
  ...
  return clamp01(
    1 - Math.log(1 + daysUntil) / Math.log(1 + PROXIMITY_HORIZON_DAYS),
  );
}

/**
 * DOMAIN_ENGINE_SPEC.md §1, amended per Lead review: a genuinely unavailable rating
 * (difficulty, self-rated confidence, grade headroom) is excluded from the sum and the
 * remaining weights are renormalized over the available mass — never defaulted to 0
 * (which silently discounts risk) or 0.5 (which fabricates an unmeasured observation).
 * `unfinished = 1.0` when nothing is planned is a real signal, not missing data, and is
 * never excluded. `procrastination` falling back to the global mean is an estimate, not
 * missing data — it stays in the sum but still downgrades confidence by one level.
 */
export function computeAssignmentRisk(input: AssignmentRiskInput): AssignmentRiskResult {
  ...
  const unfinished = input.plannedUnits === 0
    ? 1
    : clamp01(1 - input.completedUnits / input.plannedUnits);

  const congestion = clamp01(
    input.committedHours / Math.max(input.availableHours, CONGESTION_EPSILON_HOURS),
  );

  // Personal procrastination history needs >=5 observations to be trusted; below that
  // we fall back to the population mean rather than fabricating a personal figure. This
  // is an estimate, not missing data, so it stays in the weighted sum.
  const usingGlobalFallback =
    input.userMeanStartDelayDays == null ||
    (input.userStartDelaySampleSize ?? 0) < MIN_PERSONAL_START_DELAY_SAMPLE;
  const startDelayDays = usingGlobalFallback
    ? input.globalMeanStartDelayDays
    : input.userMeanStartDelayDays!;
  ...
  const base = (Object.keys(WEIGHTS) as FactorKey[]).reduce(
    (sum, key) => sum + effectiveWeights[key] * factors[key],
    0,
  );
